...
(function (S) {
  ...
  App.prototype.rebuild = function () {
    ...
    // The search box keeps its text across a rebuild, so it has to keep its
    // filter too. It used to be dropped silently: type «БЕТОН», untick a
    // street, and the box still read «БЕТОН» while the table showed all 5831
    // remaining rows and the counter agreed with the table.
    this.applySheetFilter();
    this.prices_ui.apply();
    this.fillJump();
    this.fillReportProjects();
    // The preview is a view of the model and goes stale with it. Rebuilt only
    // by the tab and the two selects, it kept counting rows an unticked street
    // had taken away — «660 строк · 637 ресурсов» over a document that had 657
    // and 634.
    if (document.getElementById('pane-report').classList.contains('on')) this.buildReportPreview();
    this.ledger();
  ...
  /**
   * Keyed by the project, not by its position. The ▲/▼ buttons reorder the
   * spans, and a selector keyed by position then quietly named a different
   * project while the rows on screen were still the previous one's.
   */
  App.prototype.fillReportProjects = function () {
    var sel = document.getElementById('reportProject');
    var cur = sel.value;
    sel.innerHTML = (this.model ? this.model.spans : []).map(function (s) {
      return '<option value="' + S.esc(s.project.file) + '">' + S.esc(s.project.name) + '</option>';
    }).join('');
  ...
  /** The span the report preview and the single-project export are about. */
  App.prototype.reportSpan = function () {
    var want = document.getElementById('reportProject').value;
    var spans = (this.model && this.model.spans) || [];
    for (var i = 0; i < spans.length; i++) if (spans[i].project.file === want) return spans[i];
    return null;
  };
  ...
  App.prototype.buildReportPreview = function () {
    ...
    var t0 = performance.now();
    var rep = S.report.build(this.model, span, this.reportOpts());
  ...
  App.prototype.loadPriceBook = function (file) {
    ...
    reader.onload = function () {
      ...
      // Through setPrices, not around it. It does everything the model needs —
      // apply, redraw, re-total — and it is the one method the server build
      // watches, so a book loaded into a workspace is saved with it and its
      // prices join the memory the next project's hints are drawn from.
      // Written straight into `prices` instead, the workspace was never marked
      // as changed: the tick stayed green over unsaved work and the whole book
      // was gone on leaving.
      self.looseBook = Object.assign(self.looseBook || {}, loose);
      var toSet = Object.assign({}, exact);
      if (self.model) {
        self.model.resources.forEach(function (r) {          // spreadLooseBook, into the map
          var p = self.looseBook[r.nk];
          if (p != null && toSet[r.key] == null && self.prices[r.key] == null) toSet[r.key] = p;
        });
      }
      self.setPrices(toSet);
