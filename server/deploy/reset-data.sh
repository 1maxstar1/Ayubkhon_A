...
ssh "$TARGET" "cd /opt/taqqoslash/server && . ./.env && \
  ...
  echo '--- zaxira'; C=\$(curl -sS \$R -o /dev/null -w '%{http_code}' -X POST \"\$BASE/api/backups\" -H \"Authorization: \$TOKEN\"); \
  echo \"backup HTTP \$C\"; \
  case \"\$C\" in 2*) ;; *) echo 'zaxira olinmadi — tozalash bekor qilindi'; exit 1;; esac; \
