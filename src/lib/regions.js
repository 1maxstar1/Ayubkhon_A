...
(function (S) {
  ...
  var PLACE = {
    andijon: 'ASAKA XONOBOD HONOBOD QORASUV KARASU SHAHRIXON SHAHRIHON BALIQCHI BULOQBOSHI IZBOSKAN ' +
      'JALAQUDUQ ZHALAKUDUK XOJAOBOD HOJAOBOD QORGONTEPA KURGANTEPA MARHAMAT MARXAMAT OLTINKOL ' +
      'PAXTAOBOD ULUGNOR QOSHTEPA',
    ...
    fargona: 'MARGILON MARGILAN QOQON KOKAND QUVASOY KUVASAY RISHTON RISHTAN BESHARIQ BUVAYDA ' +
      'DANGARA FURQAT OLTIARIQ ALTYARIK TOSHLOQ UCHKOPRIK YOZYOVON BAGDOD QOSHTEPA QUVA KUVA SOX',
    ...
    namangan: 'CHUST CHORTOQ CHARTAK KOSONSOY KASANSAY MINGBULOQ TORAQORGON TURAKURGAN UCHQORGON ' +
      'UCHKURGAN UYCHI YANGIQORGON YANGIKURGAN DAVLATOBOD NORIN POP',
    ...
    toshkent_vil: 'NURAFSHON ANGREN BEKOBOD BEKABAD CHIRCHI OLMALIQ ALMALIK OHANGARON AXANGARAN ' +
      ...
      'YUQORICHIRCHIQ NURAFSHAN TOYTEPA TUYTEPA',
  ...
  var CITY_MARK = /(?:^|[^A-Z])(SHAHRI|SHAHAR|SHAHRIDAGI|SHAHRIDA|SHAHRIN|SHAXAR|SHAXRI|GOROD|GORODA|GORODE|GORODSK)/;
  var AREA_MARK = /(?:^|[^A-Z])(VILOYAT|VILAYAT|OBLAST|OBLASTN|OBLASTI|TUMAN|TUMANI|TUMANIDA|RAYON|RAYONA|RAYONI)/;
  ...
  function scanInto(score, evidence, order, text, weight) {
    ...
    var idx = index(), i, j, joined = toks.join(' ');
    var cityMark = CITY_MARK.test(joined), areaMark = AREA_MARK.test(joined);
    ...
    for (var b = 0; b < bare.length && !tashDistrict; b++) {
      ...
      var cityHere = cityMark || CITY_MARK.test(' ' + next) || prev === 'G' || prev === 'GOR';
      var areaHere = areaMark || AREA_MARK.test(' ' + next);
